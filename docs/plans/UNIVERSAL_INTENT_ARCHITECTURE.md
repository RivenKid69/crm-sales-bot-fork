...
> **Статус:** ✅ РЕШЕНО — Теперь classifier поддерживает 150+ интентов в 26 категориях (см. `constants.yaml`)

## Проблема (историческая)

Старая архитектура имела рассинхронизацию:
- **Classifier** определял 33 интента
- **Flows** использовали 140+ интентов в transitions

Это приводило к тому, что 18 из 20 flows не работали.
