...
## 0.1 Актуальный статус (по состоянию на 2026-02-11)
...
| **Intent (намерение)** | Классифицированное намерение пользователя. Бот определяет intent на каждом ходе (247 интентов в 34 базовых категориях). Примеры: `greeting`, `price_question`, `demo_request`, `objection_price`. |
...
Чего **нет** в AI‑сервисе (ответственность внешней системы):
- Кеш сессий и управление сессиями
- Блокировки конкурентных запросов
...
> **Про config:**
>
> **Проблема:** конфигурация бота — это не один файл, а **набор из 5+ связанных
> YAML-файлов** (constants, flow, knowledge base, prompt templates, intent taxonomy),
> суммарно десятки тысяч строк. Передача всей конфигурации JSON-объектом в каждом
> запросе нецелесообразна по объёму (~50–100 KB на запрос) и потребует глубокого
> рефакторинга ядра — все компоненты (StateMachine, Classifier, Guard, LeadScorer,
> FallbackHandler, ResponseGenerator) получают `LoadedConfig` при инициализации.
>
> **Решение:** конфигурация хранится в **YAML‑файлах на стороне AI‑сервиса**.
> Каждый tenant получает свою директорию конфигов (`config/tenants/{config_name}/`).
> Внешняя система указывает `config_name` в запросе — имя конфига для загрузки.
...
| `decision_trace` | `object` | Трассировка решений (см. раздел 5). **Обязательное поле** — всегда присутствует в ответе |
...
> **Примечание:** Override вступает в силу со следующего запроса, т.к. бот пересоздаётся на каждый запрос.
...
- `decision_trace` — сохранить для аналитиков (всегда присутствует в ответе)
...
> **Важно:** API-обёртка **обязана** передавать `enable_tracing=True`
...
| Триггер | Описание |
|---|---|
| Явный запрос человека | Интенты из категории `escalation` (request_human, speak_to_manager, поступают нове интенты, запросы, при высокой фрустрации) |
