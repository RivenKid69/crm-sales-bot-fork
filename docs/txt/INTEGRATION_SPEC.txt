...
Чего **не используется** в stateless-режиме (делает внешняя система):
- Кеш сессий и SessionManager — **не задействуются** (код есть в `src/session_manager.py` как опциональный серверный кеш, но в рекомендуемом stateless-режиме не используется)
- Файловые lock-и сессий — **не задействуются** (код есть в `src/session_lock.py`, но в stateless-режиме блокировки обеспечивает внешняя система)
...
> **Примечание:** В рекомендуемом stateless-режиме override вступает в силу со следующего запроса, т.к. бот пересоздаётся на каждый запрос. Если в будущем задействуется опциональный `SessionManager` (`src/session_manager.py`) с серверным кешированием ботов, override повлияет только на **новые сессии** — кешированные продолжат работать со старыми настройками до пересоздания.
