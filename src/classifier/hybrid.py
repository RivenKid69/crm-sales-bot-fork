...
class HybridClassifier:
    ...
    def _check_demo_request_override(self, message: str, extracted: Dict) -> Optional[Dict]:
        ...
        explicit_demo_patterns = [
            ...
            # ВАЖНО: negative lookahead исключает НЕ-демо контексты
            # "хочу попробовать сам/решить/документы/конкурентов/варианты" - НЕ демо
            r'хочу\s+(?:посмотреть|попробовать|потестировать|потестить|глянуть)(?!\s+(?:сам|решить|документ|конкурент|вариант|друг|ещё|еще|на\s+друг|на\s+конкурент))',
            r'хотел\w*\s*(?:бы)?\s*(?:посмотреть|попробовать|потестировать|потестить)(?!\s+(?:сам|решить|документ|конкурент|вариант|друг|ещё|еще))',
            r'(?:можно|хотелось\s*бы)\s+(?:посмотреть|попробовать|потестировать)(?!\s+(?:документ|конкурент|вариант|друг|на\s+друг))',
            r'можно\s+показать(?:\s+(?:демо|систем|как\s+работ|crm|программ|продукт|в\s+действ))',
            r'(?:покажите|покажи)\s+(?:демо|как\s+работ|систему|продукт|в\s+действии|crm)',
            # Запись на демо
            ...
            # Посмотреть как работает — только с демо-контекстом
            # "Как это работает?" (чистый вопрос) → question_features
            # "Хочу посмотреть как работает" → demo_request
            r'(?:посмотреть|покажите|покажи|увидеть)\s+(?:как|что)\s+(?:это\s+)?работает',
            r'хочу\s+(?:узнать\s+)?(?:как|что)\s+(?:это\s+)?работает',
            r'посмотреть\s+(?:на\s+)?систем',
            r'увидеть\s+(?:как|что|интерфейс)',
            # Давай посмотрим/попробуем - исключаем НЕ-демо контексты
            r'давай\s+(?:посмотрим|глянем|попробуем)(?!\s+(?:на\s+друг|на\s+конкурент|на\s+вариант|ещё|еще|потом|позже))',
            r'давайте\s+(?:посмотрим|глянем|попробуем)(?!\s+(?:на\s+друг|на\s+конкурент|на\s+вариант|ещё|еще|потом|позже|подум))',
        ...
        # КРИТИЧЕСКИЙ FIX: Исключаем НЕ-демо контексты
        # Эти слова указывают что речь НЕ о демо системы
        non_demo_context = [
            " сам", " решить", " документ", " конкурент", " вариант",
            " друг", " ещё", " еще", " потом", " позже", " подум",
            "на друг", "на конкурент", "на вариант",
        ]
        ...
        has_non_demo = any(ctx in message for ctx in non_demo_context)

        if has_soft_demo and has_positive and not has_non_demo:
            return {
                "intent": "demo_request",
                "confidence": 0.85,
                "extracted_data": extracted,
