...
PRIORITY_PATTERNS: List[Tuple[str, str, float]] = [
    ...
    # =================================================================
    # NOTE: Сравнение "чем лучше" конкурент уже есть в CRITICAL PATTERNS (строки 27-28)
    # с confidence 0.99 и поддержкой нормализованного 'iko'. Дублирование удалено.
    (r'(?:а\s*)?(?:чем|почему)\s*(?:вы\s*)?(?:лучше|хуже)\s*(?:iiko|iko|ийко|айко|poster|постер|1с|1c)', "objection_competitor", 0.95),
    (r'(?:почему|зачем)\s*(?:вы\s*)?(?:а\s*)?не\s*(?:1с|1c|iiko|iko|ийко|poster|постер)', "objection_competitor", 0.95),
    (r'(?:у\s*)?(?:iiko|iko|ийко|айко|poster|постер)\s*(?:дешевл|лучше)', "objection_competitor", 0.95),
