...
PRIORITY_PATTERNS: List[Tuple[str, str, float]] = [
    ...
    # Стоимость/цена + количество точек/магазинов — price_question, НЕ situation_provided
    (r'(?:сколько|стоит|стоимость|цена|почём)(?:\s+\w+){0,3}\s+(?:на|для|за)\s*\d+\s*(?:точ\w*|магазин\w*|филиал\w*)', "price_question", 0.96),
    (r'(?:на|для)\s*\d+\s*(?:точ\w*|магазин\w*|филиал\w*)\s+(?:сколько|стоит|цена|почём)', "price_question", 0.96),
    (r'(?:стоить|стоит|стоимость)\s+(?:на|при|для)\s*\d+\s*(?:точ\w*|магазин\w*|филиал\w*)', "price_question", 0.96),
    ...
    # Kazakh callback/contact patterns
    (r'байланысайық', "callback_request", 0.9),      # let's connect / let's be in touch
    (r'хабарласайық', "callback_request", 0.85),      # let's communicate
    (r'қоңырау\s*шалайық', "callback_request", 0.9),  # let's call
    (r'телефон\s*соғайық', "callback_request", 0.85), # let's phone
    ...
    # ВАЖНО: \b перед "не" чтобы не матчить "не" внутри "мне", "мне нужно" и т.п.
    # =================================================================
    (r'(?:нам\s*)?(?:это\s*)?\bне\s*(?:нужно|надо|требуется)', "no_need", 0.85),
    (r'(?:обходимся|обойдёмся)\s*(?:без\s*этого)?', "no_need", 0.85),
    (r'\bне\s*вижу\s*необходимости', "no_need", 0.85),
