...
PRIORITY_PATTERNS: List[Tuple[str, str, float]] = [
    ...
    (r'(?:категорическ\w*\s*)?(?:отказываюсь|отказ)', "rejection", 0.9),
    ...
    (r'(?:закрыт|закрываемся|банкрот|ликвидац)', "rejection", 0.9),
    (r'(?:мы\s*)?(?:не\s*ваша?\s*)?(?:целевая|аудитор|клиент)', "rejection", 0.85),
    ...
    # FAREWELL (прощания)
    # =================================================================
    # КРИТИЧЕСКИЙ FIX: \bпока\b с word boundary чтобы не матчить "покажите"
    (r'(?:до\s*свидания|до\s*связи|до\s*встречи|\bпока\b|всего\s*доброго|всего\s*хорошего)', "farewell", 0.9),
