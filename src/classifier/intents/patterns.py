...
PRIORITY_PATTERNS: List[Tuple[str, str, float]] = [
    ...
    # =================================================================
    # FIX: ПРОБНЫЙ/ТЕСТОВЫЙ ПЕРИОД → payment_terms (НЕ demo_request!)
    # БД имеет 7-дневный бесплатный тестовый период.
    # "Есть тестовый период?" = вопрос об условиях, а не запрос на демо.
    # =================================================================
    (r'(?:есть|какой|сколько)\s+(?:пробн\w+|тестов\w+)\s+период', "payment_terms", 0.96),
    (r'(?:пробн\w+|тестов\w+)\s+период\w*\s*\??', "payment_terms", 0.96),
    (r'(?:попробовать|потестировать|потестить)\s+(?:перед|до)\s+(?:покупк|оплат)', "payment_terms", 0.95),
    ...
    # Общие фразы с контактом (без широкого "мои контакты", чтобы не ловить отрицания
    # вроде "не проси мои контакты")
    (r'(?:вот\s*)?(?:мой|моя)\s*(?:контакт|данные|номер|почта)', "contact_provided", 0.9),
    ...
    # "Перезвоните завтра/сегодня/в 15:00" — callback_request без "мне/нам"
    (r'(?:перезвони|позвони)\w*\s+(?:завтра|сегодня|послезавтра|утром|вечером|днём)', "callback_request", 0.92),
    (r'(?:перезвони|позвони)\w*\s+(?:в\s*\d{1,2}[:\.\s]?\d{0,2}|после\s*\d{1,2})', "callback_request", 0.92),
    ...
    (r'бесплатн\w*\s*(?:период|доступ|версию?)', "pricing_details", 0.92),
    ...
    # =================================================================
    (r'(?:чем\s*)?(?:лучше|хуже|отличаетесь?)\s*(?:от|чем)\s*(?:амо|битрикс|мегаплан|iiko|iko|постер|poster|1с|1c|crm)', "comparison", 0.9),
    ...
    # =================================================================
    # Явный ценовой якорь с бюджетным ограничением — это вопрос о цене,
    # даже если в конце есть "неинтересно".
    (r'(?:сразу|прямо)\s*(?:цена|стоимость|прайс)\b', "price_question", 0.96),
    (r'(?:цена|стоимость|прайс).{0,90}(?:если|когда)\s*(?:выше|больше)\s*\d+', "price_question", 0.96),
    (r'(?:если|когда)\s*(?:выше|больше)\s*\d+[^\n]{0,40}не\s*интересн', "price_question", 0.95),
    ...
    (r'что\s+за\s+(?:система|программа|продукт|сервис|решение)', "question_features", 0.85),
    (r'рас+каж\w*\s+(?:о|про|об)\b', "question_features", 0.9),
    (r'подробн\w*\s+(?:о|про|об)\b', "question_features", 0.85),
    (r'(?:система|программа|продукт|сервис)\s+что\s+это', "question_features", 0.85),
    ...
    (r'(?:есть\s*ли|имеется?\s*ли)\s*(?:интеграц\w*|отч[её]т\w*|поддержк\w*|api|мобильн\w*\s*приложен\w*)', "question_features", 0.8),
    (r'(?:поддержива\w*|работает\s*ли)\s*(?:ваш\w*\s+)?(?:сервис|система|программа|wipon|касса)\s*(?:с|на|в)\s*\w+', "question_features", 0.8),
    (r'(?:сервис|система|программа|wipon|касса)\s*(?:поддержива\w*|работает)\s*(?:с|на|в)\s*\w+', "question_features", 0.8),
    ...
    # =================================================================
    (r'(?:нам\s*)?(?:это\s*)?\bне\s*(?:нужно|надо|требуется)', "no_need", 0.85),
    ...
    (r'(?:тогда\s+)?зачем\s+(?:мне|нам)\s+(?:ваш|ваша|ваше|это|эта|этот)', "objection_no_need", 0.95),
    (r'(?:зачем|почему)\s+(?:мне|нам)\s+(?:платить|тратить|переплачи)', "objection_price", 0.88),
    ...
    # =================================================================
    # FRUSTRATION EXPRESSION (грубость/мат без явного no-contact)
    # ВАЖНО: directed profanity = негативная эмоция, а не терминальный
    # запрет на контакт. Явные stop-contact фразы ("не пишите", "удалите меня")
    # остаются rejection и по-прежнему ведут в soft_close.
    # =================================================================
    (r'\b(?:(?:пош[её]л|пошли)(?:\s+бы)?(?:\s+(?:ты|вы))?\s*(?:на\s*)?(?:хуй|хер|хрен|фиг)|иди(?:те)?\s*(?:на\s*)?(?:хуй|хер|хрен|фиг)|иди(?:те)?\s+лесом)\b', "frustration_expression", 0.97),
    ...
    # Kazakh patterns for objection_think
    (r'ойланамын', "objection_think", 0.90),
    (r'ойлану\s*керек', "objection_think", 0.90),
    (r'ойланып\s*к[өо]рей', "objection_think", 0.90),
    # Contract-bound refusal: explicit no-IIN boundary
    (r'(?:иин\s*не\s*дам|не\s*дам\s*иин|без\s+иин)', "objection_contract_bound", 0.96),
    (r'(?:какой|что)\s*(?:дальше|следующ\w*\s*шаг).{0,40}без\s+иин', "objection_contract_bound", 0.95),

    # =================================================================
    # PURCHASE STAGE (покупка/оплата) - ПЕРЕД agreement
    # =================================================================
    (r'(?:готов|готовы|готова)\s*(?:платить|оплатить|покупать|оформлять)', "ready_to_buy", 0.97),
    (r'(?:выставляй|выставляйте|выставьте|пришлите|присылайте|отправьте|скиньте)\s*(?:сч[её]т|инвойс)', "request_invoice", 0.97),
    (r'(?:пришлите|присылайте|отправьте|скиньте)\s*(?:договор|оферт\w*)', "request_contract", 0.96),
    (r'(?:куда\s*платить|реквизиты\s*(?:для\s*)?оплат)', "request_invoice", 0.95),
    ...
    (r'(?:думаю|наверное|пожалуй)\s+(?:можно|стоит|надо)\s+(?:попробовать|взять|подключить)', "agreement", 0.95),
    (r'(?:думаю|наверное)\s+попробую', "agreement", 0.92),
    # Заявки
    ...
    (r'(?:берём|берем|оформляем|оформляй|оформим)\s*(?:это|систему|подписку)?', "agreement", 0.95),
    (r'давайте\s+(?:оформим|оформляем|подключим|подключаем|начнём|начинаем)', "agreement", 0.95),
    ...
    # =================================================================
    # FIX: These patterns MUST be BEFORE situation_provided — first match wins!
    # =================================================================

    # "налоговая видит/получает" → question_features (НЕ data_deletion)
    (r'налогов\w+\s+(?:видит|получает|узнает|знает|увидит)', "question_features", 0.95),
    (r'(?:данные|продажи|остатки|отчёты)\s+(?:передаются|уходят|попадают)\s+(?:в\s+)?(?:налоговую|кгд)', "question_features", 0.95),
    (r'(?:кгд|налоговая)\s+(?:видит|получает|узнает)', "question_features", 0.95),

    # "ревизия/инвентаризация" questions → question_features (НЕ situation_provided)
    (r'(?:ревизи|инвентаризац)\w*\s+(?:надо|нужно|можно|закрывать|проводить|делать)', "question_features", 0.93),
    (r'(?:надо|нужно)\s+(?:закрывать|останавливать)\s+(?:магазин|точку|продажи)\s+(?:для|при|во время)', "question_features", 0.93),
    (r'(?:можно|продолжать)\s+продавать\s+(?:при|во время|пока)', "question_features", 0.93),

    # marketplace questions → question_integrations (НЕ situation_provided)
    (r'(?:можно|продавать|работать)\s+(?:на|через|с)\s+(?:ozon|wildberries|озон|вайлдберриз)', "question_integrations", 0.96),
    (r'(?:интеграци\w+)\s+(?:с\s+)?(?:маркетплейс|ozon|wildberries)', "question_integrations", 0.95),
    (r'(?:kaspi|каспи)\s+(?:магазин|qr)\s+(?:интеграц|подключ|поддержива)', "question_integrations", 0.95),
    ...
    # Extended: industry / business type descriptions
    (r'(?:мы\s*)?(?:продаём|продаем|торгуем)\s+\w+', "info_provided", 0.8),
    (r'(?:сфера|отрасль|направление)\s*[-—:]\s*\w+', "info_provided", 0.8),
    (r'(?:розница|опт|b2b|b2c|оптовая|розничная)\s*(?:торговля|продажа)?', "info_provided", 0.75),
    # Extended: tools / systems in use
    (r'(?:используем|пользуемся|работаем\s*(?:в|на|с))\s*(?:excel|1с|битрикс|амо|мегаплан)', "info_provided", 0.85),
    (r'(?:есть|имеется|используется|стоит)\s+(?:crm|срм|система|база)', "info_provided", 0.8),
    # Extended: quantitative business data
    (r'(?:продажи?|выручка|оборот)\s*(?:около|примерно|порядка)?\s*\d+', "info_provided", 0.85),
    (r'(?:штат|команда|отдел)\s*(?:из|—|:)?\s*\d+\s*(?:человек|чел)?', "info_provided", 0.8),
