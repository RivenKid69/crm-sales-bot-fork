...
PRIORITY_PATTERNS: List[Tuple[str, str, float]] = [
    ...
    # =================================================================
    # Сравнение "чем лучше/почему не" конкурент - ставим ПЕРЕД question_features!
    (r'(?:а\s+)?чем\s+(?:вы\s+)?лучше\s+(?:iiko|ийко|айко)', "objection_competitor", 0.98),
    (r'чем\s+лучше\s+(?:iiko|ийко|айко|poster|постер|1с|1c)', "objection_competitor", 0.98),
    (r'(?:а\s*)?(?:чем|почему)\s*(?:вы\s*)?(?:лучше|хуже)\s*(?:iiko|ийко|айко|poster|постер|1с|1c)', "objection_competitor", 0.95),
    (r'(?:почему|зачем)\s*(?:вы\s*)?(?:а\s*)?не\s*(?:1с|1c|iiko|ийко|poster|постер)', "objection_competitor", 0.95),
    (r'(?:у\s*)?(?:iiko|ийко|айко|poster|постер)\s*(?:дешевл|лучше)', "objection_competitor", 0.95),
    # POS и ресторанные системы
    (r'(?:уже\s*)?(?:есть|используем|работаем\s*в|сидим\s*на)\s*(?:iiko|ийко|айко|poster|постер|r.keeper)', "objection_competitor", 0.9),
    # CRM системы
...
class HybridClassifier:
    ...
    def classify(self, message: str, context: Dict = None) -> Dict:
        ...
        # =================================================================
        for pattern, intent, confidence in _COMPILED_PRIORITY_PATTERNS:
            if pattern.search(message_lower):
