...
equipment (166 секций) — оборудование и периферия:
...
tis (161 секция) — трёхкомпонентная система для ИП на упрощёнке:
  что такое ТИС, лимиты по доходу и НДС, подключение ТИС, сертификация,
  ТИС vs обычная касса, нужен ли ТИС при малых оборотах, налоговые вопросы ИП

support (121 секция) — техподдержка и обучение:
  как связаться, обучение кассиров, настройка системы, внедрение,
  консультации, удалённая помощь, выезд специалиста, docs.wipon.kz

products (88 секций) — продукты Wipon:
  Wipon Desktop (учёт на ПК), Wipon Kassa (онлайн-касса), Wipon Pro (проверка УКМ алкоголя),
  Wipon ТИС, Wipon Cashback (лояльность), что выбрать, обзор возможностей

inventory (82 секции) — товары и склад:
...
pricing (80 секций) — цены и тарифы:
  тарифы Mini/Lite/Standard/Pro, сколько стоит, стоимость подписки,
  рассрочка на оборудование, ежемесячный платёж, прайс-лист

integrations (59 секций) — интеграции:
  Kaspi.kz, Halyk Market, банки, эквайринг, POS-терминалы,
  маркировка товаров, 1С, ОФД, синхронизация заказов

features (48 секций) — функции системы:
  складской учёт, отчёты, аналитика в целом, что умеет Wipon,
  возможности программы (общие вопросы о функционале)

analytics (43 секции) — аналитика и отчёты:
  себестоимость, расчёт прибыли, отчёты по продажам, статистика,
  топ продаж, ABC-анализ, выручка, финансовые показатели

fiscal (35 секций) — фискализация:
  чеки, ОФД, отправка в налоговую, статус фискализации,
  ошибки фискализации, автопробитие чеков Kaspi, фискальный признак

regions (35 секций) — регионы и доставка:
  работаете ли в моём городе (Алматы, Астана, Шымкент, Караганда...),
  доставка оборудования, география работы, по всему Казахстану

employees (33 секции) — сотрудники и кадры:
  кассиры, отчёты по продажам сотрудника, кто сколько продал,
  история действий, ошибочное списание, доступы сотрудников

other (26 секций) — прочее:
  для кого подходит Wipon, преимущества системы, почему Wipon,
  общие вопросы о компании, сравнение с конкурентами

mobile (25 секций) — мобильное приложение:
  доступ с телефона, удалённый просмотр продаж и остатков,
  приложение iOS/Android, синхронизация с телефоном

stability (24 секции) — надёжность и стабильность:
  работа офлайн (без интернета), восстановление данных, зависания,
  резервное копирование, данные не пропадут, производительность

promotions (15 секций) — акции и скидки:
  автоматические скидки, расписание акций, скидки по времени,
  программа лояльности, бонусы, кэшбэк

faq (3 секции) — общие частые вопросы:
  нужен ли интернет, несколько устройств, миграция данных,
  нужен ли ККМ, возврат товара
