...
2. Tier 2: Semantic matching через TEI (Qwen3-Embedding-4B)
...
class IndustryDetectorV2:
    ...
    2. Semantic matching через TEI embeddings (если доступен)
    ...
    def _init_embeddings(self) -> None:
        """Pre-compute embeddings for industry profiles via TEI (с disk-кэшем)."""
        try:
            import numpy as np
            from src.knowledge.tei_client import embed_texts_cached

            texts = [profile["description"] for profile in self.INDUSTRY_PROFILES.values()]
            arr = embed_texts_cached(texts, cache_name="industry_profiles")
            if arr is None:
                return

            for i, industry in enumerate(self.INDUSTRY_PROFILES.keys()):
                self._embeddings_cache[industry] = arr[i]

            logger.debug(
                "Industry embeddings initialized via TEI",
    ...
    def _semantic_match(self, messages: List[str]) -> IndustryDetectionResult:
        ...
        Использует TEI embeddings для сравнения сообщений с профилями отраслей.
        """
        ...
        try:
            ...
            combined_text = " ".join(messages[-5:])
            if not combined_text.strip():
                return result

            query_embedding = embed_single(combined_text)
            ...
            for industry, industry_embedding in self._embeddings_cache.items():
                similarity = self._cosine_similarity(query_emb, industry_embedding)
