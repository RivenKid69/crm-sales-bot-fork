...
PERSONA_OPTION_PREFERENCES: Dict[str, List[str]] = {
    ...
    "Вернувшийся клиент": ["обновлен", "новый", "изменен", "добавил"],
    "Делегат от руководства": ["презентац", "документ", "материал", "коммерч"],
    "Перфекционист": ["подробн", "детальн", "точн", "специфик"],
    "Эмоциональный покупатель": ["понравил", "интересн", "удобн", "понять"],
    "Мульти-город предприниматель": ["несколько", "города", "точки", "филиал"],
}
...
OBJECTION_INJECTIONS: Dict[str, List[str]] = {
    ...
    "discount": ["скидка будет?", "а акция есть?", "можно дешевле по скидке?", "можно дешевле?", "у вас акция сейчас есть?"],
    "expensive": ["слишком дорого для нас", "цена завышена", "по бюджету не проходит", "150 тысяч — это много"],
    "migration": ["перенос данных будет сложный", "боюсь потерять данные при миграции", "как переносить базу без сбоев?", "как перенести номенклатуру?", "как загрузить товары из Excel?"],
    ...
    "free_alternatives": ["есть бесплатные альтернативы", "зачем платить, если есть free-решения?", "мы можем на бесплатном обойтись", "а бесплатная версия есть?", "зачем платить если Kaspi касса бесплатная?", "есть пробный период?"],
    ...
    "simplicity": ["слишком сложно для команды", "нужно проще", "персонал не разберется", "слишком сложно для продавца", "персонал не разберётся", "нужно попроще"],
...
но потом верни разговор к своему вопросу."""
        ...
        # Kazakh speaker: inject language instruction so LLM actually uses Kazakh words
        if self.persona.name == "Казахоязычный":
            prompt += "\nВАЖНО: Ты казахоязычный клиент. Используй казахские слова вместо русских: иә (да), жоқ (нет), жаксы (хорошо), рахмет (спасибо), болады (ладно/можно). Можно писать короткие фразы на казахском."
    ...
    def _is_contact_request(self, bot_message: str) -> bool:
        ...
        patterns = [
            ...
            # Kaspi-specific patterns for autonomous_closing payment path
            r"kaspi\s+(?:номер|телефон)",
            r"номер\s+kaspi",
            r"kaspi[-\s]?телефон",
            r"(?:ваш|свой)\s+(?:номер\s+)?kaspi",
            r"ИИН\s+(?:и|для|нужен|укаж)",
            r"(?:ИИН|инн)\s*(?:и\s+)?(?:номер|телефон)?",
            r"(?:номер|телефон)\s+и\s+ИИН",
            r"для\s+оформлени(?:я|и)\s+(?:счёт|оплат|kaspi)",
            r"(?:счёт|оплат).{0,30}(?:ИИН|телефон|номер)",
        ]
    ...
    def _handle_contact_request(self) -> str:
        ...
        # "Недоверчивый" не даёт контакты пока не убеждён (первые 4 хода)
        if self.persona.name == "Недоверчивый" and self.turn <= 4:
            refusals = [
                "пока не готов давать контакты",
                "сначала убедите что это надёжно",
                "хочу сначала разобраться, потом контакты",
                "а зачем вам мой номер сейчас?",
            ]
            return random.choice(refusals)

        # "Готов купить" сразу даёт телефон + ИИН — как в реальных диалогах
    ...
    def should_continue(self) -> bool:
        ...
        if self.persona.name in ["Занятой", "Агрессивный", "Просто смотрю", "Фрустрированный ожидающий"]:
    ...
    def _generate_leave_message(self) -> str:
        ...
        leave_messages = {
            ...
            "Готов купить": ["ок, жду счёт", "хорошо, свяжитесь", "понял, напишу потом"],
            "Казахоязычный": ["жаксы, рахмет", "ок рахмет", "болды, потом"],
            "Недоверчивый": ["подумаю", "нужно проверить", "не уверен пока"],
            "Фрустрированный ожидающий": ["всё, надоело ждать", "сколько можно", "не буду больше ждать"],
            "Нишевый бизнес": ["ладно, изучу сам", "потом вернусь", "посмотрю другие варианты"],
