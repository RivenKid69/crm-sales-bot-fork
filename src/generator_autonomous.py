...
15. Если в подтвержденных фактах несколько разных сущностей с похожим именем — реши по контексту диалога о чём клиент. Если контекст не помогает — уточни коротко."""
...
HARD_NO_CONTACT_MARKERS: Tuple[str, ...] = (
    ...
    "контакт бермеймін",
    "контакт бермеймiн",
    "қазір контакт бермеймін",
    "қазір контакт бермеймiн",
    "телефон бермеймін",
    "телефон бермеймiн",
)
...
def build_state_gated_rules(
    ...
    if state_value == "autonomous_discovery" and intent_value in {
        ...
        n_turns = len(history) if isinstance(history, list) else 0
        if n_turns < 3:
            rules.append((1,
                "⚠️ DISCOVERY: клиент только описал ситуацию. "
                "НЕ предлагай тариф и цену. Прояви эмпатию и задай ОДИН уточняющий вопрос "
                "про боли или задачи (что мешает, что хотят автоматизировать). "
                "Тариф будет позже."
            ))
    ...
    if (
        ...
        rules.append((2,
            ...
            "Сначала дай прямой ответ на текущий вопрос по подтвержденным фактам, "
...
def _build_name_markers(name: str) -> Tuple[str, ...]:
    """Build stable markers for detecting prior client-name mentions in bot history."""
...
def _bot_response_mentions_name(bot_text: str, markers: Tuple[str, ...]) -> bool:
    """Return True when bot_text addresses the client by one of the known name markers."""
    low = str(bot_text or "").lower()
...
def count_name_mentions_in_history(
    history: list,
    name: str,
    recent_bot_turns: Optional[int] = None,
) -> int:
    """Count how many prior bot turns already used the client's name."""
    markers = _build_name_markers(name)
    ...
    if recent_bot_turns and recent_bot_turns > 0:
        bot_turns = bot_turns[-recent_bot_turns:]

    return sum(1 for bot_text in bot_turns if _bot_response_mentions_name(bot_text, markers))
...
def build_address_instruction(
    ...
    if name:
        recent_mentions = count_name_mentions_in_history(history or [], name, recent_bot_turns=4)
        if recent_mentions >= 1:
            return (
                f'ОБРАЩЕНИЕ: клиента зовут "{name}". '
                "В ЭТОМ ответе НЕ используй имя: оно уже звучало в одном из последних 4 ответов. "
                "Продолжай по сути и без обращения по имени."
            )
        return (
            f'ОБРАЩЕНИЕ: клиента зовут "{name}". '
            "Если обращаешься по имени, делай это максимум 1 раз за 4 ответа. "
            f'НЕ начинай ответ с имени и НЕ повторяй имя в этом сообщении. Без "господин/госпожа" — просто "{name}".'
        )
...
def build_language_instruction(user_message: str) -> str:
    ...
    if (kz_letters or has_kz_words) and ru_letters:
        return (
            ...
            "Отвечай простым русским. НЕ смешивай языки и НЕ вставляй казахские слова."
        )
    if kz_letters or has_kz_words:
        return (
            "⚠️ ЯЗЫК: Клиент пишет на казахском. Отвечай простым русским, "
            "без казахских вставок и без смешения языков в одном предложении."
        )
...
def build_stress_instruction(intent: str, frustration_level: int, user_message: str) -> str:
    ...
    if (
        ...
        instructions.append(
            "РЕЖИМ КРАТКОСТИ: 1-2 предложения, сначала ключевой подтвержденный факт. "
...
def enforce_no_contact_boundaries(text: str, context: Dict[str, Any]) -> str:
    ...
    patterns = (
        ...
        r"(?i)\s*остав(?:ьте|ь|ите)(?:,\s*пожалуйста)?[^?.!]{0,40}(?:контакт|номер|телефон|email)[^?.!]*[?.!]",
        r"(?i)\s*уточн(?:ите|и)(?:,\s*пожалуйста)?[^?.!]{0,50}(?:контакт|номер|телефон|email)[^?.!]*[?.!]",
        r"(?i)\s*(?:пришл(?:ите|и)|напиш(?:ите|и)|укаж(?:ите|и)|дайте)\s*(?:почту|email)[^?.!]*[?.!]",
        r"(?i)\s*(?:для\s+связи|для\s+оформления)[^?.!]{0,35}(?:остав(?:ьте|ь)|укаж(?:ите|и))[^?.!]*(?:телефон|email|контакт)[^?.!]*[?.!]",
