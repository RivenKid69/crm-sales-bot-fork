...
def build_language_instruction(user_message: str) -> str:
    ...
    if (kz_letters or has_kz_words) and ru_letters:
        return (
            ...
            "Отвечай на РУССКОМ, но начни с 1 казахского слова-приветствия (сәлем, иә, жақсы). "
            "Короткие предложения, без повторов."
        )
    if kz_letters or has_kz_words:
        return (
            "⚠️ ЯЗЫК: Клиент пишет на КАЗАХСКОМ. ОБЯЗАТЕЛЬНО отвечай на РУССКОМ, "
            "но начни с краткого казахского приветствия или подтверждения (Иә, Жақсы, Түсінікті). "
            "Модель не владеет казахским свободно — лучше понятный русский ответ, чем ломаный казахский. "
            "НЕ пытайся писать длинные фразы на казахском."
        )
