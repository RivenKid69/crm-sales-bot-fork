"""
Маркеры тона для Tier 1 (regex-based) анализа.

Содержит:
- TONE_MARKERS: Словарь паттернов для каждого типа тона
...
TONE_MARKERS: Dict[Tone, List[str]] = {
    ...
    Tone.INTERESTED: [
        r"расскажите\s+подробнее", r"а\s+как",
...
# Веса для накопления frustration
